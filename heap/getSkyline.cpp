...
vector<pair<int, int> > getSkyline(vector<vector<int> >& buildings) {
    ...
    int curX, curH, cur = 0;
    while( cur < buildings.size() || !liveBuildings.empty() ) {
      if( liveBuildings.empty() || ( cur < buildings.size() && buildings[cur][0] <= liveBuildings.top().second ) ) {
        ...
        curX = liveBuildings.top().second;
        while( !liveBuildings.empty() && liveBuildings.top().second <= curX )
          liveBuildings.pop();
      }
      curH = liveBuildings.empty() ? 0 : liveBuildings.top().first;
